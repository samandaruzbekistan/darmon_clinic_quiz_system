...
@section('section')
    <div class="dash-content">
        <div class="overview">
            ...
            <form action="{{ route('vacant_test_check') }}" method="post">
                @csrf
                <input type="hidden" name="vacant_id" value="{{ $vacant->id }}">
                <input type="hidden" name="vacant_fullname" value="{{ $vacant->fullname }}">
                <h1 style="text-align: center; margin-top: 1rem; color: blue">{{ $subjects[0]->name }}</h1>
                ...
                @endforeach
                    ...
                    <h1 style="text-align: center; margin-top: 1rem; color: blue">{{ $subjects[1]->name }}</h1>
                    @foreach ($second as $id => $quiz)
                        @if($quiz->photo != "no_photo")
                            <div class="big_test">
                                <section class="test">
                                    <div class="test_questions">
                                        <div>
                                            <h2 class="test_request"><b>{{ $id+11 }}. </b> {{ $quiz->quiz }}</h2>
                                            @foreach ($first_answer[$id] as $item)
                                                <label>
                                                    <input type="radio" name="answer{{ $id+11 }}" value="{{ $item->id }}">
                                                    <span>{{ $item->answer }}</span>
                                                </label>
                                            @endforeach
                                        </div>
                        ...
                        @else
                            <div class="small_tests">
                                <section class="small_test">
                                    <div>
                                        <h2 class="test_title">{{ $id+11 }}. </b> {{ $quiz->quiz }}</h2>
                                        @foreach ($first_answer[$id] as $item)
                                            <label>
                                                <input type="radio" name="answer{{ $id+11 }}" value="{{ $item->id }}">
                                                <span>{{ $item->answer }}</span>
                        ...
                        <hr>
                    @endforeach
                    <h1 style="text-align: center; margin-top: 1rem; color: blue">{{ $subjects[2]->name }}</h1>
                    @foreach ($third as $id => $quiz)
                        @if($quiz->photo != "no_photo")
                            <div class="big_test">
                                <section class="test">
                                    <div class="test_questions">
                                        <div>
                                            <h2 class="test_request"><b>{{ $id+21 }}. </b> {{ $quiz->quiz }}</h2>
                                            @foreach ($first_answer[$id] as $item)
                                                <label>
                                                    <input type="radio" name="answer{{ $id+21 }}" value="{{ $item->id }}">
                                                    <span>{{ $item->answer }}</span>
                                                </label>
                                            @endforeach
                                        </div>
                        ...
                        @else
                            <div class="small_tests">
                                <section class="small_test">
                                    <div>
                                        <h2 class="test_title">{{ $id+21 }}. </b> {{ $quiz->quiz }}</h2>
                                        @foreach ($first_answer[$id] as $item)
                                            <label>
                                                <input type="radio" name="answer{{ $id+21 }}" value="{{ $item->id }}">
                                                <span>{{ $item->answer }}</span>
                ...
                <h1 style="text-align: center; margin-top: 1rem; color: blue">{{ $subjects[3]->name }}</h1>
